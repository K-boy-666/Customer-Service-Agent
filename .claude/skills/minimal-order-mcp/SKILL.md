...
description: >
  ...
  MCP tools for Claude. Use this skill whenever the user wants to build an order query system,
  create an MCP server for orders, wrap an order API for AI agents, scaffold an order
  management backend, or mentions "order MCP", "订单 MCP", "expose order data to Claude",
  "给 Claude 加订单查询", "order tools for agent", "AI 查订单", or any three-tier
...
```
Agent (Claude)  ──stdio──▶  MCP Server (server.py)
                                  │ HTTP
                                  ▼
...
which is what Claude Code and Claude Desktop expect.
...
# Terminal 2: Claude Code auto-launches the MCP server via settings.json
claude
```

### 6.2 Verification checklist

In Claude Code, ask these questions and confirm sensible answers:
...
| MCP tools don't appear | settings.json not loaded | Restart Claude Code after editing settings |
...
- `references/settings_template.json` — Claude Code configuration
